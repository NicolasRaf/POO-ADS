...
class Bank {
    ...
    getClientndexByNumber(clientCPF) {
        for (let i = 0; i < this.accounts.length; i++) {
            if (this.clients[i].getCPF() == clientCPF)
                return i;
        }
    }
    ...
    deleteClient(clientCPF) {
        let clientIndex = this.getAccountIndexByNumber(clientCPF);
        if (clientIndex !== undefined) {
            const indexArraySize = this.clients.length - 1;
    ...
    linkClientToAccount(accNumber, clientCPF) {
