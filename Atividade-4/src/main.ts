import { Hotel, Radio, Account, Greeting, Triangule, Equipament, UpdatedAccount, Player } from './classes/index';
...
function main(){
    ...
    let r : Radio = new Radio(15); // Adicionado valor passado ao construtor
    r.volume = 10;
    ...
    let c1: Account = new Account("1",100);
    let c2: Account = new Account("2",100);
    let c3: Account;
    ...
    const happyGreeting = new Greeting("Bom dia","Ely");
    console.log(happyGreeting.getGreeting()); // print da saudação com os parâmetros passados

    // 7ª Questão 
    printNumberQuestion(7);
    const notTri = new Triangule(3,20,1)
    const equiTri = new Triangule(5, 5, 5);
    const escTri = new Triangule(3, 4, 5);
    const isoTri = new Triangule(5, 7, 5);
    ...
    let upC1 = new UpdatedAccount("1", 90);
    let upC2 = new UpdatedAccount("2", 50);
    ...
    const jogador1 = new Player(10, 5, 100); // Força 10, Nível 5, Pontos 100
    const jogador2 = new Player(8, 4, 80);   // Força 8, Nível 4, Pontos 80
